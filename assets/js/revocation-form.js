/**
 * GutenBlock Pro – Revocation (withdrawal) Form frontend.
 *
 * Client-side validation, AJAX submit, double-submit protection and replacing
 * the form with the success message. Supports multiple forms per page.
 * Configuration comes from the localized `gutenblockProRevocationForm`.
 */
( function () {
	'use strict';

	var config = window.gutenblockProRevocationForm || {};
	...
	function setFieldError( field, message ) {
		...
		msg.className = 'gbp-rf-error';
		msg.textContent = message;
		var wrap = field.closest( '.gbp-rf-field' ) || field.parentNode;
	...
	function clearFieldError( field ) {
		...
		var wrap = field.closest( '.gbp-rf-field' ) || field.parentNode;
		var existing = wrap.querySelector( '.gbp-rf-error' );
		if ( existing ) {
			existing.remove();
		}
	}
	...
	function showFeedback( wrap, message, isError ) {
		var box = wrap.querySelector( '.gbp-rf-feedback' );
		if ( ! box ) {
			return;
		}
		box.textContent = message;
		box.classList.toggle( 'gbp-rf-feedback--error', !! isError );
		box.hidden = false;
	}

	function replaceWithSuccess( wrap, message ) {
		var success = document.createElement( 'div' );
		success.className = 'gbp-rf-success';
	...
	function resetButton( form, button ) {
		...
		var label = button.querySelector( '.gbp-rf-submit-label' );
	...
	function handleSubmit( e ) {
		...
		var wrap = form.closest( '.gbp-revocation-form-wrap' );
		var button = form.querySelector( '.gbp-rf-submit' );
	...
	function init() {
		var forms = document.querySelectorAll( '.gbp-revocation-form-wrap .gbp-revocation-form' );
